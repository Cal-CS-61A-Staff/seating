...
def get_content_charset(headers):
    try:
        get_param = headers.get_param  # Python 3
    except AttributeError:
        get_param = headers.getparam  # Python 2
...
def fetcher(q):
    ...
    while 1:
        ...
        response = None
        try:
            response = urlopen(url, cookie=cookie)
        ...
        if response and response.getcode() == 200:
            ...
            ext = "".join(
                sorted(
                    mimetypes.guess_all_extensions(response.headers["Content-Type"]),
                    key=lambda ext: mime_type_ranks.index(ext.lower())
                    if ext.lower() in mime_type_ranks
                    else len(mime_type_ranks),
                )[:1]
            )
            if len(data) > 0:
                ...
                if not os.path.exists(path_with_ext):
                    ...
                    try:
                        ...
                        if not (
                            ex.errno == errno.EEXIST and os.path.isdir(path_parent)
                        ):
                            raise
...
def find_cookies_in_cookies_txt(for_domain, content):
    ...
    for match in re.finditer(
        ...
        if for_domain == domain or ("." + for_domain).endswith(
            "." + domain.lstrip(".")
        ):
...
ROSTER_FIELDS = [
    ...
    "units",
    "sections",
]  # Not set in stone: the server may add/remove fields as it wishes...


def main(program, *args):
    program_name = os.path.basename(program)
    browser_cookie_module_name = (
        "browser_cookie3" if sys.version_info[0] >= 3 else "browsercookie"
    )
...
""".format(
            ...
            roster_fields="  " + ", ".join(ROSTER_FIELDS),
            default_format_field=DEFAULT_FORMAT_FIELD,
    ...
    argparser.add_argument(
        "--format",
        required=False,
        help='the photo file name format string (default: "%%(default)s")\n(example: "%s")'
        % ("{id} - {first_name} {last_name} - {student_id} - {email}",),
        default=DEFAULT_FORMAT_FIELD,
    )
    argparser.add_argument(
        "--file",
        required=False,
        action="store_true",
        help="indicates cookie parameter is a file name rather than the cookie string itself",
    )
    argparser.add_argument(
        "bcourses_cookie",
        nargs="?",
        help="bCourses cookie string (default) or cookie file (if --file is specified)\r\ncontaining the %s cookie for %s"
        % (repr(cookie_name), server),
    )
    ...
    if bcourses_cookie is None:
        sys.stderr.write(
            "Automatically extracting %r cookies from the browser..." % (server,)
        )
        sys.stderr.flush()
        try:
            try:
                browser_cookie_module = __import__(browser_cookie_module_name)
            except ImportError:
                browser_cookie_module = None
            if browser_cookie_module is None:
                msg = "failed to import {module}; please install it: {python} -m pip install {package}".format(
                    python=PYTHON,
                    module=browser_cookie_module_name,
                    package=browser_cookie_module_name,
                )
                raise ImportError(msg)
            bcourses_cookie = "; ".join(
                map(
                    lambda cookie: "%s=%s" % (cookie.name, cookie.value),
                    browser_cookie_module.load(server)
                    if browser_cookie_module_name == "browser_cookie3"
                    else filter(
                        lambda cookie: cookie.domain == server,
                        browser_cookie_module.load(),
                    ),
                )
            )
        finally:
            if bcourses_cookie:
                sys.stderr.write(" success! %s" % (bcourses_cookie,))
            else:
        ...
        if (
            ";" not in bcourses_cookie
            and " " not in bcourses_cookie
            and "=" not in bcourses_cookie[:48]
        ):
            bcourses_cookie = cookie_name + "=" + bcourses_cookie
            sys.stderr.write(
                "WARNING: Cookie string doesn't appear to include %s; assuming you forgot to include it...\n"
                % (repr(cookie_name + "="),)
            )
    ...
    sys.stderr.writelines(["Downloading course info..."])
    sys.stderr.flush()
    course_info = read_http_response_as_json(
        urlopen(
            server_with_protocol + "/api/academics/rosters/canvas/embedded",
            cookie=bcourses_cookie,
        )
    )
    canvas_course = course_info.get("canvas_course")
    if canvas_course is None:
        msg = (
            "failed to retrieve course information; please verify the %s cookie is correct: %s"
            % (repr(cookie_name), repr(bcourses_cookie))
        )
    ...
    q = Queue()
    threads = list(
        map(lambda i: threading.Thread(target=fetcher, args=(q,)), range(nthreads))
    )
    for t in threads:
        t.setDaemon(True)
        t.start()
    sys.stderr.writelines(["Reading student emails..."])
    sys.stderr.flush()
    lines = None
    ...
    for line in lines:
        ...
        if found is not None and "photo" in found:
            path = os.path.join(
                ...
                % (
                    (
                        list(map(lambda section: section["name"], found["sections"]))
                        + [course_name]
                    )[0],
                    course_id,
                ),
                format_string.format(**found),
            )
